...
plt.plot(t_data, y_norm, 'o-', label='IFN-γ Data (normalised)')
plt.plot(t_data, y_fit_norm, 's--', label='EIM Fit (normalised)')
plt.xlabel('Time (h post-challenge)')
plt.ylabel('Normalised IFN-γ')
plt.title('Normalized Shape Fit of EIM (IFN-γ vs y(t))')
