# =============================================================================
#
#  Full Integrated Script (Final Version 2.0):
#  - Implements proportional VNS suppression.
#  - Uses rescaled time-course data from Koopman et al. (2016) for v(t).
...
# =============================================================================
#  PART 2: SIMULATE VNS EFFECT (USING RESCALED KOOPMAN DATA)
# =============================================================================

print("\n--- Starting Part 2: Simulating VNS Effect (with Rescaled Koopman Data) ---")

# 1) Build the v(t) temporal template using Koopman et al. data
days = np.array([0, 42, 56, 84])
suppression_norm = np.array([0.0, 1.0, 0.052, 0.791])
hours = days * 24

# *** MODIFIED: Rescale the time axis to fit the 0-100 hour simulation window ***
# The original template spans 0 to 2016 hours. We map this to 0-100.
# We will make the effect start after a small delay (e.g., 2 hours)
hours_rescaled = np.interp(hours, [hours.min(), hours.max()], [2, 100])

# Use PCHIP interpolator for a shape-preserving curve
# We use the rescaled time axis for our new v(t) template
...
# 4) Show graphical plot (removed u(t) plot as requested)
