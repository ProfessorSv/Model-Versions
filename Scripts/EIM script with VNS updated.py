# =============================================================================
#
#  Full Integrated Script (Final Version):
#  - Implements proportional VNS suppression to ensure non-negativity.
#  - Uses IFN-gamma data from Drewes et al. (2021) for the v(t) template.
#  - Simulation time is set to 100 hours to focus on initial dynamics.
#
...
#  PART 3: Output results to terminal (metrics and ASCII plot)
#
# =============================================================================
...
# NOTE: Update the file path below to match the location of your 'il4_hdm.csv' file.
try:
...
# =============================================================================
#  PART 2: SIMULATE VNS EFFECT (USING DREWES ET AL. 2021 DATA)
# =============================================================================

print("\n--- Starting Part 2: Simulating VNS Effect (with Drewes et al. Data) ---")

# 1) Build the v(t) temporal template from Drewes et al. (2021) IFN-gamma data
# Baseline: 29.8, Day 2: 18.4, Day 5: 22.5
days = np.array([0, 2, 5])
ifn_gamma_values = np.array([29.8, 18.4, 22.5])

# Calculate suppression and normalize to peak = 1
baseline_ifn = ifn_gamma_values[0]
suppression = 1 - (ifn_gamma_values / baseline_ifn)
v_t_normalized = suppression / np.max(suppression)
hours = days * 24

# Use PCHIP interpolator for a shape-preserving curve
v_interp = PchipInterpolator(hours, v_t_normalized, extrapolate=False)
...
# 4) Show graphical plot
fig, axs = plt.subplots(2, 2, figsize=(14, 10))
fig.suptitle('EIM Simulation: Effect of VNS on a Calibrated Model (0-100 hours)', fontsize=16)
...
axs[1, 1].plot(sol_baseline.t, sol_baseline.y[3], label='Baseline', lw=2); axs[1, 1].plot(sol_vns.t, sol_vns.y[3], label='VNS', lw=2, linestyle='--')
axs[1, 1].set_title('External Lesion'); axs[1, 1].set_xlabel('Time (hours)'); axs[1, 1].set_ylabel('u(t)'); axs[1, 1].legend(); axs[1, 1].grid(True, linestyle=':')
