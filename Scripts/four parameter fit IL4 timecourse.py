...
# 4) Simulator at the exact data time points, with variable u0
initial_state = [0, 0, 0, 1.0]
def simulate(p, u0):
    sol = solve_ivp(
        lambda t, s: EIM_ODEs(t, s, p),
...
# 5) Residuals fitting δ, Λ1, Λ2, and u0
def residuals(x):
    ...
    p['Lambda1'] = Λ1_val
    p['Lambda2'] = Λ2_val
    return simulate(p, u0_val) - y_data

# 6) Perform the four-parameter fit
initial_guess = [
    params_base['delta'],
    params_base['Lambda1'],
    params_base['Lambda2'],
    1.0  # initial guess for u0
]
bounds = ([0, 0, 0, 0], [np.inf, np.inf, np.inf, np.inf])
res = least_squares(residuals, initial_guess, bounds=bounds)
δ_fit, Λ1_fit, Λ2_fit, u0_fit = res.x
print(f"Fitted δ = {δ_fit:.4f}, Λ₁ = {Λ1_fit:.4f}, Λ₂ = {Λ2_fit:.4f}, u₀ = {u0_fit:.2f}")

# 7) Generate fitted curve & compute fit metrics
...
ax.set_title('Four-Parameter Fit of Engram-Immune Model')
textstr = (
    f'δ={δ_fit:.4f}, Λ₁={Λ1_fit:.4f}, Λ₂={Λ2_fit:.4f}, u₀={u0_fit:.2f}\n'
...
ax2.set_title('Residuals of Four-Parameter Fit')
ax2.legend()
plt.tight_layout()
plt.show()
# =====================  END SCRIPT  =====================
# This script fits a four-parameter model to IL-4 time-course data using the Engram-Immune ODE system.
# It uses the least-squares method to optimize the parameters and visualizes the results.