...
# 1. Load & normalise Eotaxin-1 data
data      = pd.read_csv(r'C:\Users\celal\Desktop\Model-Versions\data\Eotaxin-1_hdm.csv')
t_data    = data['time'].values
y_raw     = data['Eotaxin-1'].values
...
plt.plot(t_data, y_norm, 'o-', label='Eotaxin-1 Data (normalised)')
plt.plot(t_data, y_fit_norm, 's--', label='EIM Fit (normalised)')
plt.xlabel('Time (h post-challenge)')
plt.ylabel('Normalised Eotaxin-1')
plt.title('Normalized Shape Fit of EIM (Eotaxin-1 vs y(t))')
...
ss_tot_norm = np.sum((y_norm - np.mean(y_norm))**2)
