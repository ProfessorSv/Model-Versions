...
# 1. Load & normalise TARC data
data      = pd.read_csv(r'C:\Users\celal\Desktop\Model-Versions\data\TARC_hdm.csv')
t_data    = data['time'].values
y_raw     = data['TARC'].values
...
plt.plot(t_data, y_norm, 'o-', label='TARC Data (normalised)')
plt.plot(t_data, y_fit_norm, 's--', label='EIM Fit (normalised)')
plt.xlabel('Time (h post-challenge)')
plt.ylabel('Normalised TARC')
plt.title('Normalized Shape Fit of EIM (TARC vs y(t))')
